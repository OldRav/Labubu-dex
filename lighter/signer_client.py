...
def _get_arch_name():
    ...
    if is_arm and is_mac:
        ...
        return "signer-arm64.dylib"
    elif is_linux and is_x64:
        logging.debug("Detected x64/amd architecture on Linux.")
        return "signer-amd64.so"
    elif is_windows:
        logging.debug("Detected architecture on Windows.")
        return "signer-amd64.dll"
    else:
        raise Exception(
            f"Unsupported platform/architecture: {platform.system()}/{platform.machine()} only supports Linux(x86) and Darwin(arm64)"
        )

def _get_arch_path(account_index: int):
    arch_file_name = _get_arch_name()

    current_file_directory = os.path.dirname(os.path.abspath(__file__))
    path_to_signer_folders = os.path.join(current_file_directory, "signers")
    path_to_temp_signer = os.path.join(path_to_signer_folders, "temp_signers")
    if not os.path.isdir(path_to_temp_signer):
        os.mkdir(path_to_temp_signer)

    original_arch_file = os.path.join(path_to_signer_folders, arch_file_name)
    temp_arch_file = os.path.join(path_to_temp_signer, f"{account_index}-" + arch_file_name)
    return original_arch_file, temp_arch_file

def _initialize_signer(account_index: int):
    if DLL_LOADED.get(account_index):
        return DLL_LOADED[account_index]

    original_arch_file, temp_arch_file = _get_arch_path(account_index)
    if not os.path.exists(temp_arch_file):
        shutil.copy2(original_arch_file, temp_arch_file)

    signer_dll = ctypes.CDLL(temp_arch_file)
    DLL_LOADED[account_index] = signer_dll
    return signer_dll

def _remove_temp_arch_file(account_index: int, handle: int):
    _, temp_arch_file = _get_arch_path(account_index)
...
def _remove_all_temp_arch_files():
    current_file_directory = os.path.dirname(os.path.abspath(__file__))
    path_to_signer_folders = os.path.join(current_file_directory, "signers")
    path_to_temp_signer = os.path.join(path_to_signer_folders, "temp_signers")

    temp_files_to_remove = os.listdir(path_to_temp_signer)
    for temp_file_name in temp_files_to_remove:
        try:
            os.remove(os.path.join(path_to_temp_signer, temp_file_name))
        except Exception as err:
            if "Access is denied" in str(err):
                account_index = int(temp_file_name.split('-')[0])
                signer_dll = _initialize_signer(account_index)
                _remove_temp_arch_file(account_index=account_index, handle=signer_dll._handle)


def create_api_key(seed="", account_index: int = 0):
    signer = _initialize_signer(account_index=account_index)
...
class SignerClient:
    ...
    def __init__(
        ...
        api_key_index,
        account_index,
        api_client: lighter.ApiClient = None,
        max_api_key_index=-1,
        ...
        self.signer = _initialize_signer(account_index=account_index)
        self.api_client = api_client or lighter.ApiClient(configuration=Configuration(host=url))
